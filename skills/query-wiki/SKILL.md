---
name: query-wiki
description: Use this skill ONLY when the user explicitly invokes structured vault querying — synthesize across pages with mandatory inline `[[Page]]` citations, optionally surface comparison tables or Mermaid diagrams, and proactively offer to graduate the answer back into the wiki as a new page when it has reuse value (3+ sources synthesized, new framing, or likely re-asked). Reading is hybrid: index-first walk via root → wiki-index → wiki-page chain (per Karpathy LLM Wiki canonical), with a grep fallback through obsidian-cli search when index walk misses keywords. English triggers (REQUIRED to invoke this skill) include "ask the wiki", "query wiki", "what does my wiki say about X", "/query-wiki". Japanese triggers (REQUIRED) include "wiki に質問", "wiki に聞く", "wiki から探して", "クエリ", "/query-wiki". Bare questions like "頭痛と耳鳴りの共通点は？" do NOT trigger this skill — that informal vault consultation is handled by the Vault-First Consultation rule (see CANONICAL.md), which is Claude-side default behavior, not a skill invocation. The /query-wiki skill is the structured-query path with mandatory citations and graduation handoff. Requires kepano/obsidian-skills (hard dependency) and a running Obsidian app.
---
