...
- `/add-page` でページを追加、`/lint-vault` で schema/hygiene/structural チェック (auto-fix 付き)、`/query-wiki` で vault に質問 + 引用付き回答 + graduation で wiki 化、ができます (kepano/obsidian-skills が必要、Obsidian 起動中に実行)。`/daily-log` は v0.1.1+ で optional add-on として予定
