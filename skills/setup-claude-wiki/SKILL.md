...
- `/add-page` でページを追加できます (kepano/obsidian-skills が必要、Obsidian 起動中に実行)。`/query-wiki`, `/lint-vault` は今後のプラグイン更新で届きます。`/daily-log` は v0.1.1+ で optional add-on として予定
...
- `/add-page` is available for adding pages (requires kepano/obsidian-skills with Obsidian running). `/query-wiki`, `/lint-vault` ship in upcoming plugin releases. `/daily-log` is planned for v0.1.1+ as an optional add-on.
...
- `/add-page` でページを追加できます (kepano/obsidian-skills が必要)。`/query-wiki`, `/lint-vault` は次のプラグイン更新で届きます (`/daily-log` は v0.1.1+ optional)
```
...
- `/add-page` is available for adding pages (requires kepano/obsidian-skills). `/query-wiki`, `/lint-vault` arrive in upcoming plugin updates (`/daily-log` is optional, planned for v0.1.1+)
```
