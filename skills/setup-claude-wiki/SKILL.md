...
Read the user's reply as a list of domain names. Do not transform their wording — preserve their exact spelling and language. If the user provides fewer than 2 or more than 8, ask them to refine concretely:

- **Fewer than 2:** "1 つだけだと wiki のうま味が出にくいです。他に追跡したい領域はありますか？" (suggest at least 1 more)
- **More than 8:** "少し多めなので、似た領域を一つにまとめられないか確認させてください。たとえば『読書』+『リサーチ』を『学び』に、『買い物』+『家計』を『お金』に、のような統合が可能か？" (offer concrete merger candidates from their list)
...
Lead the proposal with the **4-section pattern** below. It matches how working professionals usually organize knowledge: `Inbox` for unsorted, `Work` for professional content, `Private` for personal content, `System` for cross-domain notes.

**Slotting rule for the user's Step 5 domains:**

- **Work**: paid work, career development, job-specific projects (e.g. `business`, `coding`, `meetings`, `clients`)
- **Private**: personal interests, health, family, hobbies (e.g. `health`, `reading`, `cooking`, `family`)
- **System**: cross-cutting reference / glossary content that doesn't belong cleanly to either side (e.g. `obsidian-tips`, `wiki-rules`)

When ambiguous (e.g. `reading` could be professional learning OR personal hobby), ask:「これは仕事のスキルとして追跡しますか？それとも個人の楽しみとして？」 and slot per the answer. Don't auto-decide silently.
...
The full text of each template is in **`references/templates.md`** — read that file when you need the exact body to write.

Templates available:

- **Template 7.A** — `README.md` (vault root; `type: wikiページ`/`wiki-page`, orientation page)
- **Template 7.B** — `Inbox/Inbox.md` (root-index)
- **Template 7.B-2** — `<System>/<System>.md` (root-index, mandatory)
- **Template 7.C** — `<top-folder>/<top-folder>.md` (root-index, for nested mode top folders)
- **Template 7.D** — `<domain>/<domain>.md` (wiki-index, JP `索引` / EN `wiki-index`, with flat-mode variant)
- **Template 7.E** — `<domain>/<domain>-log.md` (wiki-log, with `init` log entry)

Each template pairs JP and EN versions; pick the locale chosen in Step 3.
...
- `/add-page` adds pages, `/lint-vault` runs schema/hygiene/structural health checks with auto-fix, and `/query-wiki` queries the vault with cited synthesis + graduation back to a wiki page (all require kepano/obsidian-skills with Obsidian running). `/daily-log` is planned for v0.1.1+ as an optional add-on.
