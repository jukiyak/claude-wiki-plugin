---
name: lint-vault
description: Use this skill when the user wants to health-check or audit a claude-wiki vault — find schema, hygiene, and structural issues across 17 rules (missing frontmatter, retired keys, enum drift, orphan pages, dead wikilinks, sub-wiki threshold, file-naming mismatches, etc.). Presents a Batch Approval Plan grouped by rule and applies auto-fixable corrections on approval (frontmatter populate, retired-key drop, date quoting, missing-field defaults); manual-review items are surfaced but not modified. English triggers include "lint vault", "audit vault", "health check vault", "/lint-vault". Japanese triggers include "vault を lint", "vault チェック", "vault 監査", "リント", "ヘルスチェック", "/lint-vault". Requires kepano/obsidian-skills (hard dependency) and a running Obsidian app.
---
...
Skip this rule for vaults in EN locale (detected via the canonical locale-detect procedure: `${CLAUDE_PLUGIN_ROOT}/CANONICAL.md` → Wiki Page Frontmatter → Locale Mapping → Locale auto-detect).

#### A6 — Locale consistency

Within one file, all frontmatter keys should be JP or all EN — never mixed (e.g. `タイプ: ...\ntags: ...`). Mixed → **review**, with a **suggested target locale** based on the **vault-wide majority vote** from the canonical locale-detect procedure (`${CLAUDE_PLUGIN_ROOT}/CANONICAL.md` → Wiki Page Frontmatter → Locale Mapping → Locale auto-detect). User decides whether to accept the suggestion or pick the other locale; v0.2.0+ will ship an automated migration skill that applies this choice atomically.
